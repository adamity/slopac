...
else
{
	if(isset($_GET['search']))
	{
		...
		$query = "SELECT * FROM borrower WHERE member_id LIKE '%$search%' OR accession_num LIKE '%$search%' OR borrow_date LIKE '%$search%' OR return_date LIKE '%$search%'";
		$result = mysqli_query($link,$query)or die("Query failed!");

		if(mysqli_num_rows($result)>0)
		{
			$displayBook = "<tr><th>MEMBER ID</th><th>ACCESSION NUMBER</th><th>BORROW DATE</th><th>RETURN DATE</th></tr>";
			...
			while($row = mysqli_fetch_array($result))
			{
				...
				$displayBook = $displayBook . "<tr><td>".$member_id."</td><td>".$accession_num."</td><td>".$borrow_date."</td><td>".$return_date."</td></tr>";
			}
